...
def test_resource_policy_with_config(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Positive test - WITH labels and resources
    positive_yaml = f"""
apiVersion: v1
kind: Pod
metadata:
...
"""
    ...
    if ok(r):
        tests.append(create_test_result(
            ...
            f"Resource policy - positive test (labels: {labels})",
            True,
            f"Pod with resources created successfully in {test_ns}",
            "INFO"
        ))
        run_command(f"kubectl delete pod {test_name}-valid -n {test_ns} --ignore-not-found=true", timeout=10)
    else:
    ...
    # Negative test - WITH labels but WITHOUT resources
    negative_yaml = f"""
apiVersion: v1
kind: Pod
metadata:
...
"""
    ...
    if not ok(r) and ('denied the request' in r.get('combined', '')):
        tests.append(create_test_result(
            ...
            f"Resource policy - negative test (labels: {labels})",
            True,
        ...
        if ok(r):
            ...
            tests.append(create_test_result(
                ...
                False,
                f"Failed but not due to policy: {error_msg[:200]}",
                "WARNING"
            ))
...
def test_security_context_policy_with_config(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Positive test - WITH labels and proper security context
    positive_yaml = f"""
apiVersion: v1
kind: Pod
metadata:
...
"""
    ...
    if ok(r):
        tests.append(create_test_result(
            ...
            f"Security context - positive test (labels: {labels})",
            True,
            f"Pod with security context created successfully",
            "INFO"
        ))
        run_command(f"kubectl delete pod {test_name}-valid -n {test_ns} --ignore-not-found=true", timeout=10)
    else:
    ...
    # Negative test - WITH labels but WITHOUT security context
    negative_yaml = f"""
apiVersion: v1
kind: Pod
metadata:
...
def test_pod_level_security_policy_with_config(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Positive test - WITH labels and NO host settings
    positive_yaml = f"""
apiVersion: v1
kind: Pod
metadata:
...
"""
    ...
    if ok(r):
        tests.append(create_test_result(
            ...
            f"Pod security - positive test (labels: {labels})",
            True,
            f"Pod without host settings created successfully",
            "INFO"
        ))
        run_command(f"kubectl delete pod {test_name}-valid -n {test_ns} --ignore-not-found=true", timeout=10)
    else:
    ...
    # Negative test - WITH labels and hostNetwork=true
    negative_yaml = f"""
apiVersion: v1
kind: Pod
metadata:
